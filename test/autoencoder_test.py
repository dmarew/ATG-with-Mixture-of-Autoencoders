...
for index, image  in enumerate(sim_data_loader):
    if index > 0:
        ...
        #plt.show()
    if index == 0 or np.max(recon_ll) < 0.6:
    ...
    obs_save_dir_full = '../data/can_online_testing/can_online_testing_0/dataset/by_aspect/'+ str(c_aspect_node) + '/'
...
with open('can.pickle', 'wb') as handle:
